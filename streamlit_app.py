...
st.markdown("""
    ...
    div[data-testid="column"]:nth-child(2) button { background-color: transparent; border: 1px solid #ddd; font-size: 14px; height: 2.5em; }
    div[data-testid="column"]:nth-child(3) button { height: 2.5em; font-size: 14px; }
    .cat-header { background-color: #f0f2f6; padding: 3px 10px; border-radius: 8px; border-left: 5px solid #005bac; margin: 15px 0 5px 0; font-size: 14px; font-weight: bold; }
    .money-box { background-color: #fff1f1; padding: 12px; border-radius: 12px; border: 2px solid #ff4b4b; margin-bottom: 15px; }
...
with tab1:
    ...
    # 合計の計算（current_priceがあればそれを使い、なければlast_priceを初期値として計算）
    total_spent = 0
    for idx, row in buying_df.iterrows():
        # 買い物リストでの入力値を優先し、なければ前回価格を使う
        price = row.get('current_price') if pd.notnull(row.get('current_price')) else row['last_price']
        total_spent += int(price) if str(price).isdigit() else 0
    ...
    if buying_df.empty:
        ...
        for idx, row in buying_df.iterrows():
            ...
            # 初期値として「前回価格(last_price)」を表示
            # すでに今入力中の値(current_price)があればそれを使う
            display_price = row.get('current_price') if pd.notnull(row.get('current_price')) else row['last_price']
            
            p_input = c2.text_input("円", value=str(display_price), key=f"t_{idx}", label_visibility="collapsed")
            
            if p_input != str(display_price):
                df.at[idx, 'current_price'] = int(p_input) if p_input.isdigit() else 0
                data["inventory"] = df.to_dict(orient="records")
                save_all_data(data)
                st.rerun()
                
        if st.button("🎉 買い物完了", type="primary"):
            # 完了時に current_price を last_price に上書きして保存
            for idx in df[df['to_buy'] == True].index:
                price = df.at[idx, 'current_price'] if pd.notnull(df.at[idx, 'current_price']) else df.at[idx, 'last_price']
                df.at[idx, 'last_price'] = price
                df.at[idx, 'current_price'] = None # 一時保存用をクリア
            ...
            if not cat_df.empty:
                ...
                for idx, row in cat_df.iterrows():
                    ...
                    if cols[2].button(btn_txt, key=f"a_{idx}"):
                        ...
                        # 買うボタンを押した瞬間に current_price をリセット
...
with tab3:
    with st.form("new"):
        ...
        if st.form_submit_button("登録"):
            if n:
                new_item = {"name": n, "cat": c, "to_buy": False, "last_price": 0, "current_price": None}
